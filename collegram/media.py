...
def preprocess_from_message(
    ...
    if media is not None:
        if isinstance(media, MessageMediaPhoto):
            ...
            isinstance(media, MessageMediaWebPage)
            and getattr(media.webpage, "cached_page", None) is not None
        ):
            # Empty cached_page parts to lighten messages, save it in media folder.
            page_save_path = (
                media_save_path / "cached_pages" / f"{media.webpage.id}.json"
            )
